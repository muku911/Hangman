...
public class ForHardLevel : MonoBehaviour {
	...
	private void Kata_Kata(int number) {
		...
		if (number == 1 ) setKalimat ("Ketoprak","Makanan Khas Jakarta");
		else if (number == 2 )setKalimat ("CheesseBurger","Dari Amerika");
		else if (number == 3 )setKalimat ("Hamburger","Makanan Dari jerman");
		else if (number == 4 )setKalimat ("Bibimbab","Campuran daging dan Sayur (Korea)");
		else if (number == 5 )setKalimat ("Brownies","Jenis Kue Coklat");
		else if (number == 6 )setKalimat ("Bulgogi","Secara Harfiah Artinya Api Dan Daging (Korea)");
		else if (number == 7 )setKalimat ("Croissant","Kue yg di  sirami Mentega isi selai rasberry(Prancis)");
		else if (number == 8 )setKalimat ("Lasagna","Hidangan Tradisional (Italia)");
		else if (number == 9 )setKalimat ("DIMSUM","Yang Berarti Sentuh Hatimu (Hongkong)");
		else if (number == 10 )setKalimat ("Pierogi","bentuknya seperti pastel tutup Indonesia (Polandia)");
	}
